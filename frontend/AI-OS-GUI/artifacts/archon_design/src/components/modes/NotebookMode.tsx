...
import AnalyticsDashboard from "../notebook/AnalyticsDashboard";
...
function NotebookModeContent() {
  const { selectedCitation, setSelectedCitation, isStudioOpen, activeTab, currentNotebookId } = useNotebookContext();
  ...
  return (
    <div className="flex h-full w-full overflow-hidden bg-app-bg text-text-primary">
      ...
      {activeTab === "chat" ? (
        ...
        <AnalyticsDashboard notebookId={currentNotebookId ?? ""} />
      )}
