...
export default function ObsidianMode() {
  ...
  const [filter, setFilter] = useState<Skill["category"] | "all">(() => {
    try {
      const saved = sessionStorage.getItem("archon_obsidian_filter");
      return (saved as Skill["category"] | "all") || "all";
    } catch {
      return "all";
    }
  });

  const handleSetFilter = (newFilter: Skill["category"] | "all") => {
    setFilter(newFilter);
    try {
      sessionStorage.setItem("archon_obsidian_filter", newFilter);
    } catch (e) {}
  };
