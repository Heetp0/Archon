...
// ── Agent mode: multi-file tabs ────────────────────────────────────────────────
interface FileTab { id: string; name: string; icon: React.ElementType; iconColor: string; content: React.ReactNode; }

const FILE_TABS: FileTab[] = [
  { id: "analysis", name: "analysis.py", icon: FileCode, iconColor: "text-blue-400", content: (
  ...
  { id: "schema", name: "schema.sql", icon: Database, iconColor: "text-purple-400", content: (
    <div className="p-4 font-mono text-xs text-slate-500 text-center py-8">
      No schema generated yet.
    </div>
  )},
  { id: "config", name: "config.json", icon: FileJson, iconColor: "text-yellow-400", content: (
  ...
  { id: "readme", name: "README.md", icon: FileText, iconColor: "text-slate-300", content: (
    <div className="p-4 font-mono text-xs text-slate-500 text-center py-8">
      ...
      {mode === "agents" && (
        <div className="flex flex-col flex-1 overflow-hidden">
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    active ? "bg-[#020817] border-b-2 border-b-green-500" : "text-slate-600 hover:text-slate-400 hover:bg-slate-900/40"
                  )}
                >
          ...
          <div className="flex-1 overflow-hidden bg-[#0d1117] relative">
            ...
            {openTabs.length === 0 ? (
              <div className="flex items-center justify-center h-full text-slate-700 font-mono text-xs">No files open</div>
            ) : activeTabData ? (
              <ScrollArea className="h-full">{activeTabData.content}</ScrollArea>
            ) : null}
          </div>

          {/* Context Files at bottom */}
