...
class VaultSearch:
    ...
    def index_vault(self):
        ...
        with self.lock:
            ...
            indexed_hashes = {}
            try:
                # Query only relative_path and content_hash columns to be memory-efficient
                res = self.table.search().select(['relative_path', 'content_hash']).to_list()
                for row in res:
            ...
            for note in notes:
                ...
                if rel_path not in indexed_hashes or indexed_hashes[rel_path] != note['content_hash']:
                    ...
                    if rel_path in indexed_hashes:
                        ...
                        self.table.delete(f'relative_path = "{escaped_path}"')
            ...
            for del_path in deleted_paths:
                escaped_del_path = del_path.replace('"', '\\"')
                self.table.delete(f'relative_path = "{escaped_del_path}"')
